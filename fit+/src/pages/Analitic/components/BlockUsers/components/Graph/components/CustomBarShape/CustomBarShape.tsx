...
const CustomBarShape: React.FC<BarProps & { activeIndex: number | null }> = (props) => {
  const { x, y, width, height, fill, background, index, activeIndex, payload } = props as any;
  const value = payload?.value ?? 0;
  const bgY = background && typeof background === 'object' && 'y' in background ? Number(background.y) : Number(y);
  const bgHeight = background && typeof background === 'object' && 'height' in background ? Number(background.height) : 0;
  const radius = 8;
  ...
  const bgFill = activeIndex === index ? 'rgba(214,255,0,0.15)' : BACKGROUND_COLOR;
